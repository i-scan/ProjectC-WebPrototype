...
assert(script.includes('VAL-012-UT3'), 'Momentum ruleset marker is missing')
assert(script.includes('momentum-collision-lab-v1'), 'Momentum implementation marker is missing')
assert(script.includes('惯性实验室'), 'UT3 inertia lab is missing')
assert(script.includes('AT PHASE'), 'phase-by-phase AT playback is missing')
assert(script.includes('momentumByActorId'), 'actor Momentum indicator data is missing')
assert(script.includes('Active Momentum'), 'visible Active Momentum marker is missing')
