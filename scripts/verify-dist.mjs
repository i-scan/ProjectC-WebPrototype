...
assert(script.includes('VAL-012-UT4'), 'UT4 ruleset marker is missing')
assert(script.includes('coupled-inertia-sandbox-v1'), 'UT4 implementation marker is missing')
assert(script.includes('惯性实验室 · Hex6 Layout'), 'UT4 Hex6-shaped route is missing')
assert(script.includes('Player Actions'), 'UT4 persistent action hand is missing')
assert(script.includes('UT4 Thermal'), 'UT4 controlled pendulum marker is missing')
assert(script.includes('Thermal Debug'), 'UT4 Thermal Debug controls are missing')
assert(script.includes('Spatial Debug'), 'UT4 Spatial Debug controls are missing')
assert(script.includes('Action / Event Log'), 'UT4 diagnostic log is missing')
assert(script.includes('Hold Position'), 'UT4 Hold Position action is missing')
assert(script.includes('Heavy Release'), 'UT4 Heavy Release action is missing')
assert(script.includes('受击 / Hit Player'), 'UT4 Inject Hit control is missing')
assert(script.includes('Queue Dummy Move'), 'UT4 queued dummy control is missing')
assert(script.includes('Damping'), 'UT4 damping control is missing')
...
assert(/\.coupled-inertia-lab/.test(style), 'UT4 sandbox styling is missing from CSS')
assert(/\.ut4-action-hand/.test(style), 'UT4 action hand styling is missing from CSS')
assert(/\.ut4-controlled-pendulum/.test(style), 'UT4 pendulum styling is missing from CSS')
