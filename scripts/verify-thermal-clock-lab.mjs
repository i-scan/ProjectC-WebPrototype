...
try {
  ...
  const result = spawnSync(chromeExecutable(), [
    ...
    '--window-size=1800,1200', '--virtual-time-budget=1400', '--run-all-compositor-stages-before-draw', '--dump-dom', pageUrl,
  ...
  assert(dom.includes('data-thermal-pendulum="temperature-relative-set-point-v1"'), 'Thermal Pendulum marker missing')
  ...
  assert(dom.includes('Preview worldAt remains 0.0 until Commit.'), 'Preview must not advance worldAt')
  ...
  assert(dom.includes('data-thermal-integer-ghosts'), 'Integer AT ghost readout missing')
  ...
  console.log('Thermal Clock Lab browser smoke verified: isolated analytic solver, Pendulum ghosts, 7 Drift cards, environment/dynamics controls, diagnostics, Preview/Commit contract and reserved board are mounted.')
} finally {
