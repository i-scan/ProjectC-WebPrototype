...
try {
  ...
  await waitFor('UT7 Vite preview', async () => { const response = await fetch(pageUrl, { redirect: 'follow' }); if (!response.ok) throw new Error(`HTTP ${response.status}`); return true })

  const userDataDir = join(tmpdir(), `projectc-ut7-inertia-driving-${process.pid}`)
  chromeProcess = spawn(chromeExecutable(), ['--headless=new', '--no-sandbox', '--disable-gpu', '--hide-scrollbars', '--remote-debugging-address=127.0.0.1', '--remote-debugging-port=9228', `--user-data-dir=${userDataDir}`, '--window-size=1366,1080', 'about:blank'], { stdio: ['ignore', 'ignore', 'pipe'] })
  chromeProcess.stderr.on('data', (chunk) => process.stderr.write(`[ut7-chrome] ${chunk}`))
  const version = await waitFor('UT7 Chrome DevTools', async () => { const response = await fetch(`${debuggingOrigin}/json/version`); if (!response.ok) throw new Error(`HTTP ${response.status}`); return response.json() })
  ...
  await waitFor('UT7 live root', async () => {
    ...
    if (!ready) throw new Error('UT7 root / 3D board canvas not mounted'); return true
  })
  await sleep(450)
  ...
  await waitFor('UT7 2D target field', async () => { const snapshot = await evaluate(client, snapshotExpression); if (snapshot.validTargets < 20) throw new Error(JSON.stringify(snapshot)); return snapshot })
  ...
  const routePreview = await waitFor('UT7 route inspector', async () => { const snapshot = await evaluate(client, snapshotExpression); if (snapshot.routeRows < 1 || !snapshot.route.includes('ETA') || !snapshot.route.match(/use|resist|generate/)) throw new Error(JSON.stringify({ snapshot, targetProbe })); return snapshot })
  ...
  const reverseTargetReady = await waitFor('UT7 exact reverse target', async () => {
    ...
    if (!exists) throw new Error('Expected W3 reverse target (4,7) is not selectable'); return true
  })
  ...
  const branchChoice = await waitFor('UT7 180-degree branch choice', async () => { const snapshot = await evaluate(client, snapshotExpression); if (!snapshot.branchVisible || snapshot.branchCount !== 2 || !snapshot.header.includes('0.0 AT')) throw new Error(JSON.stringify(snapshot)); return snapshot })

  await evaluate(client, `document.querySelector('[data-ut7-branch="cw"]').click(); true`)
  const afterReverse = await waitFor('UT7 committed reverse steering', async () => { const snapshot = await evaluate(client, snapshotExpression); if (snapshot.header.includes('0.0 AT') || snapshot.branchVisible || !snapshot.latestLog.match(/Steer \/ Horizontal|Steer \/ Generate/)) throw new Error(JSON.stringify(snapshot)); return snapshot })

  const beforeEnemyToggleHeader = afterReverse.header
  await evaluate(client, `document.querySelector('.ut7-actor-loop [data-control="spawn-enemies"]').click(); true`)
  const enemiesOff = await waitFor('UT7 pure-driving enemy toggle', async () => { const snapshot = await evaluate(client, snapshotExpression); if (snapshot.spawnEnemiesOn || snapshot.actorOptionCount !== 1 || snapshot.header !== beforeEnemyToggleHeader) throw new Error(JSON.stringify(snapshot)); return snapshot })
  ...
  const radius10 = await waitFor('UT7 radius rebuild', async () => { const snapshot = await evaluate(client, snapshotExpression); if (snapshot.radius !== 10 || snapshot.actorOptionCount !== 1 || !snapshot.header.includes('0.0 AT')) throw new Error(JSON.stringify(snapshot)); return snapshot })

  await mkdir(artifactDir, { recursive: true })
  const screenshot = await client.send('Page.captureScreenshot', { format: 'png', captureBeyondViewport: false })
  await writeFile(join(artifactDir, 'ut7-inertia-driving.png'), Buffer.from(screenshot.data, 'base64'))
  const result = { initial, routePreview, reverseTargetReady, branchChoice, afterReverse, enemiesOff, radius10 }
  await writeFile(join(artifactDir, 'ut7-inertia-driving.json'), `${JSON.stringify(result, null, 2)}\n`)

  console.log('UT7 inertia driving verified in real Chrome: target preview, explicit 180-degree branch choice, committed steering, Spawn Enemies preservation, and radius rebuild.')
