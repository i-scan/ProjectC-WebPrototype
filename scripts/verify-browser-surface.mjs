...
const snapshotExpression = `(() => {
  ...
  return {
    ...
    axisUi:root?.dataset.axisUi??'', boardAxisDirection:board?.dataset.axisDirection??'', boardAxisStyle:board?.dataset.axisStyle??'', actorAxisPersistent:board?.dataset.actorAxisPersistent??'',
    ...
    previewStyle:board?.dataset.previewStyle??'', previewArrow:board?.dataset.previewArrow??'', reachableHighlight:board?.dataset.reachableHighlight??'', knockbackPlayback:board?.dataset.knockbackPlayback??'',
...
async function setKinematics(client, axisId, level) {
  ...
  await waitFor(`kinematics ${axisId} M${level}`, async () => {
    const value = await snapshot(client)
...
async function moveFreeM0(client, q, r) {
  ...
  await idleAt(client, current.worldAt + 1)
}
function integerHexFromPosition(position) {
  const r = Math.round(position.z / 0.8660254037844386)
  const q = Math.round(position.x - r * 0.5)
  const centerX = q + r * 0.5
  const centerZ = r * 0.8660254037844386
  const exact = Math.hypot(position.x - centerX, position.z - centerZ) < 0.03
  return exact ? { q, r } : null
}
...
try {
  ...
  await client.open(); await client.send('Page.enable'); await client.send('Runtime.enable'); await client.send('Page.bringToFront')
  ...
  const initial = await waitFor('initialized visible map', async () => {
    const value = await snapshot(client)
    if (value.implementation !== 'cell-world-spatial-ab-v3' || value.viewportWidth < 700 || value.viewportHeight < 300 || value.canvasWidth < 700 || value.canvasHeight < 300) throw new Error(JSON.stringify(value))
    return value
  })
  ...
  await resetUi(client); await setAction(client, 'basic-move'); await setKinematics(client, 'E', 2)
  assert(await evaluate(client, 'window.__PROJECTC_PROTOTYPE__.fireAt(2,0)'), 'M2 forward rejected')
  const afterM2 = await idleAt(client, 1)
  assert(afterM2.momentum === 1 && afterM2.axisId === 'E', 'M2 Range2 must spend to M1', afterM2)

  await resetUi(client); await setAction(client, 'basic-move'); await setKinematics(client, 'E', 3)
  assert(await evaluate(client, 'window.__PROJECTC_PROTOTYPE__.fireAt(3,0)'), 'M3 forward rejected')
  const afterM3 = await idleAt(client, 1)
  assert(afterM3.momentum === 2 && afterM3.axisId === 'E', 'M3 Range3 must spend to M2', afterM3)

  await resetUi(client); await setKinematics(client, 'E', 2)
  ...
  const afterHold = await idleAt(client, 1)
  assert(afterHold.momentum === 1 && Math.abs(afterHold.logicalX) < 0.02 && Math.abs(afterHold.logicalZ) < 0.02, 'Hold must stay in Cell and dissipate M2→M1', afterHold)

  await resetUi(client); await setAction(client, 'basic-move'); await setKinematics(client, 'E', 2)
  assert(await evaluate(client, 'window.__PROJECTC_PROTOTYPE__.fireAt(-1,0)') === false, 'reverse Move accepted')
  const afterReverse = await snapshot(client)
  ...
  const duringChain = await waitFor('staged chain playback', async () => {
    const value = await snapshot(client)
    if (!value.playing || value.playerPlaybackProgress < 0.99 || value.actorPlaybackWindowCount < 3) throw new Error(JSON.stringify(value))
    return value
  })
  const chainEvents = await evaluate(client, 'window.__PROJECTC_PROTOTYPE__.conflicts()')
  const primaryTransfer = chainEvents.find((event) => event.kind === 'momentum-transfer' && event.sourceActorId === 'player')
  assert(primaryTransfer?.sourceBeforeM === 2 && primaryTransfer?.sourceAfterM === 1 && primaryTransfer?.targetAfterM === 2, 'M exchange regressed', chainEvents)
  const afterChain = await idleAt(client, 1)
  ...
  assert(JSON.stringify(chainCells) === JSON.stringify({ 'dummy-a': '4,1', 'dummy-b': '5,1', 'dummy-c': '6,1' }) && afterChain.momentum === 1, 'chain final state regressed', { chainCells, afterChain })
  ...
  const wallData = await waitFor('rendered wall contact trajectory', async () => {
    ...
    const path = paths['dummy-a'] ?? []
    const hasFractionalContact = path.some((point) => !Number.isInteger(point.q) || !Number.isInteger(point.r))
    if (!hasFractionalContact || !path.some((point) => point.q === 2 && point.r === 0)) throw new Error(JSON.stringify(paths))
    return paths
  })
  const wallEvents = await evaluate(client, 'window.__PROJECTC_PROTOTYPE__.conflicts()')
  const wallCrash = wallEvents.find((event) => event.kind === 'wall-crash' && event.actorId === 'dummy-a')
  ...
  assert(wallEvents.some((event) => event.kind === 'surface-stop' && event.actorId === 'dummy-a'), 'blocked head-on rebound must stop', wallEvents)
  assert(!wallEvents.some((event) => event.kind === 'surface-reflection' && event.actorId === 'dummy-a'), 'blocked head-on rebound must not side-branch', wallEvents)
  const afterWall = await idleAt(client, 1)
  const wallState = await evaluate(client, 'window.__PROJECTC_PROTOTYPE__.snapshot()')
  assert(wallState.actors[0].hex.q === 2 && wallState.actors[0].hex.r === 0 && Math.abs(afterWall.logicalX - 1) < 0.02 && Math.abs(afterWall.logicalZ) < 0.02, 'wall contact produced a player/target swap', { wallState, wallData, afterWall })

  // Exact latest regression: put the player at q2,r1, then hit the thin Hard Wall
  // at q3,r0 with NE/M3. The broad z+ wall face must turn the first reflected
  // logical Cell immediately to SE (q2,r2), with no q3,r1 detour.
  await resetUi(client); await setCollisionSurfaces(client, false); await setAction(client, 'basic-move'); await setAtMs(client, 250)
  await moveFreeM0(client, 1, 0); await moveFreeM0(client, 1, 1); await moveFreeM0(client, 2, 1)
  await setCollisionSurfaces(client, true); await setKinematics(client, 'NE', 3); await setAction(client, 'basic-move')
  const obliqueReach = await evaluate(client, 'window.__PROJECTC_PROTOTYPE__.reachability()')
  assert(obliqueReach.some((entry) => (entry.targetHex?.q ?? entry.finalHex?.q) === 2 && (entry.targetHex?.r ?? entry.finalHex?.r) === 1), 'oblique wall collision Cell is not clickable', obliqueReach)
  ...
  const obliqueSamples = await waitFor('oblique rendered-wall reflection', async () => {
    ...
    const collisionIndex = samples.findIndex((sample) => sample.collision)
    ...
    if (!integerAfter.length || integerAfter[0].q !== 2 || integerAfter[0].r !== 2) throw new Error(JSON.stringify({ integerAfter, samples }))
    if (integerAfter.some((hex) => hex.q === 3 && hex.r === 1)) throw new Error(JSON.stringify({ integerAfter }))
    return samples
  })
  const afterOblique = await idleAt(client, 4)
  assert(afterOblique.axisId === 'SE', 'oblique wall reflection did not preserve reflected Axis', afterOblique)

  await resetUi(client); await setAtMs(client, 300)
  assert(await evaluate(client, `window.__PROJECTC_PROTOTYPE__.setSpatialMode('hybrid')`), 'Hybrid switch failed')
  await setAction(client, 'drive'); await setKinematics(client, 'E', 1)
  assert(await evaluate(client, 'window.__PROJECTC_PROTOTYPE__.fireAt(0,-2)'), 'Hybrid Drive rejected')
  const hybridSamples = await waitFor('Hybrid samples', async () => {
    const samples = await evaluate(client, 'window.__PROJECTC_PROTOTYPE__.trajectory()')
    if (samples.length < 100) throw new Error(`samples=${samples.length}`)
    return samples
  })
  await idleAt(client, 1)

  await mkdir(artifactDir, { recursive: true })
  const screenshot = await client.send('Page.captureScreenshot', { format: 'png', captureBeyondViewport: false })
  await writeFile(join(artifactDir, 'rendered-wall-reflection.png'), Buffer.from(screenshot.data, 'base64'))
  const evidence = { initial, afterM2, afterM3, afterHold, afterReverse, duringChain, primaryTransfer, chainCells, wallData, wallEvents, afterWall, obliqueSamples, afterOblique, hybridSampleCount: hybridSamples.length }
  await writeFile(join(artifactDir, 'rendered-wall-reflection.json'), `${JSON.stringify(evidence, null, 2)}\n`)
