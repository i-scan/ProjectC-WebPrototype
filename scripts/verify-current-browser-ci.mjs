...
const chromeDebugPort = process.env.BROWSER_VERIFY_DEBUG_PORT || '9229'
...
function cleanupStaleChrome() {
  ...
  const pattern = `--remote-debugging-port=${chromeDebugPort}`
  const result = spawnSync('pkill', ['-f', '--', pattern], { stdio: 'ignore' })
  if (result.error && result.error.code !== 'ENOENT') {
    console.warn(`Unable to clean stale Chrome verifier process: ${result.error.message}`)
  }
}

function runVerifier() {
  return new Promise((resolve) => {
    const child = spawn(process.execPath, ['scripts/verify-ut7-basic-move.mjs'], {
      env: process.env,
...
for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
  ...
  if (!chromeStartupFailure || attempt === maxAttempts) process.exit(result.code)

  console.warn(`Chrome startup was not ready; cleaning stale current-verifier Chrome before retry (${attempt}/${maxAttempts}).`)
  cleanupStaleChrome()
