...
describe('browser verification CI resilience', () => {
  it('routes historical Hex checks through the retry wrapper and then verifies live UT5 and UT6', () => {
    expect(packageJson.scripts['verify:browser']).toBe('node scripts/verify-browser-layout-ci.mjs && node scripts/verify-ut5-axis-inertia.mjs && node scripts/verify-ut6-actor-loop.mjs')
    ...
    expect(ut6).toContain('UT6 Actor Loop verified in real Chrome')
  })
