...
const ut4InteractionPath = fileURLToPath(new URL('./verify-ut4-interactions.mjs', import.meta.url))
const ut4RuntimePath = fileURLToPath(new URL('./verify-ut4-runtime-feedback.mjs', import.meta.url))
const packageJson = JSON.parse(readFileSync(packagePath, 'utf8'))
const wrapper = readFileSync(wrapperPath, 'utf8')
const ut4 = readFileSync(ut4Path, 'utf8')
const ut4Interaction = readFileSync(ut4InteractionPath, 'utf8')
const ut4Runtime = readFileSync(ut4RuntimePath, 'utf8')
...
describe('browser verification CI resilience', () => {
  it('routes Hex checks through the transient-startup retry wrapper and then verifies UT4', () => {
    expect(packageJson.scripts['verify:browser']).toBe('node scripts/verify-browser-layout-ci.mjs && node scripts/verify-ut4-sandbox.mjs && node scripts/verify-ut4-interactions.mjs && node scripts/verify-ut4-runtime-feedback.mjs')
    ...
    expect(ut4Interaction).toContain('UT4 player Movement Axis arrow')
    expect(ut4Interaction).toContain('UT4 first continuous Basic Move')
    expect(ut4Interaction).toContain('UT4 Drive board targets')
    expect(ut4Interaction).toContain('UT4 neutral reset baseline')
    expect(ut4Runtime).toContain('Nobody Dies should refill HP')
    expect(ut4Runtime).toContain('UT4 diagnostic surfaces are not visibly labelled')
    expect(ut4Runtime).toContain('UT4 Hot Drive Movement M')
  })
