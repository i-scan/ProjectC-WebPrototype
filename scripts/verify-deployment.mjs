...
for (let attempt = 1; attempt <= attempts; attempt += 1) {
  try {
    ...
    if (info.implementation !== 'continuous-inertia-v1') throw new Error(`published implementation is ${info.implementation}`)
    ...
    if (!bundle.includes('continuous-inertia-v1')) throw new Error('published bundle is not continuous inertia v1')
    console.log(`Verified production main@${expectedCommit.slice(0, 8)}`)
    process.exit(0)
  } catch (error) {
    lastError = error
