...
await retry('published JavaScript bundle', async (attempt) => {
  ...
  // Current production entry: UT6-style shell + impulse/aim movement input.
  if (!script.includes('impulse-inertia-input-v1')) throw new Error('impulse input implementation marker is missing')
  ...
  if (!script.includes('Click Cell = Aim only')) throw new Error('aim-not-target interaction marker is missing')
  if (!script.includes('Apply Impulse · Resolve 1 AT')) throw new Error('explicit impulse commit control is missing')
...
await retry('published stylesheet', async (attempt) => {
  ...
  if (!/\.impulse-commit-row/.test(style)) throw new Error('impulse commit styling is missing')
