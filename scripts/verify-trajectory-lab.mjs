...
try {
  ...
  assert(dom.includes('data-motion-freeze="m2"'), 'Default M2 high-speed frozen-frame marker missing')
  assert(dom.includes('Click Cell = direction only, never Destination'), 'Direction-not-destination readability marker missing')
  assert(dom.includes('data-axis-style="actor-body-screen-arrow-v5"'), 'Board3D Axis HUD did not mount')

  console.log('Trajectory Lab browser smoke verified: Process Steering B, Action-complete Ready, dual projection and frozen-speed presentation are mounted.')
} finally {
