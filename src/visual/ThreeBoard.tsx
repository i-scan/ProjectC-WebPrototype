...
function cellColor(cell: Cell) {
  ...
  const normalized = Math.max(-3, Math.min(3, cell.groundTemp)) + 3
...
function createOverlay(color: number, opacity: number, height: number) {
  const geometry = new THREE.PlaneGeometry(0.82, 0.82)
...
function createCloud(cell: Cell, bobAnimations: BobAnimation[]) {
  ...
  bobAnimations.push({ object: group, baseY: 2.15, phase: Math.random() * Math.PI * 2, amplitude: 0.08, speed: 1.1 })
  return group
}
...
function createActorPawn(actor: GameState['actors'][number], billboardGroups: THREE.Group[]) {
  ...
  const body = new THREE.Mesh(new THREE.CylinderGeometry(0.22 * bodyScale, 0.29 * bodyScale, 0.58 * bodyScale, 16), primary)
  ...
  if (actor.actorType === 'player') {
    const sword = new THREE.Mesh(new THREE.BoxGeometry(0.05, 0.48, 0.1), new THREE.MeshStandardMaterial({ color: 0xe7edf5, metalness: 0.75, roughness: 0.2 }))
  ...
  if (actor.actorType === 'elite') {
    const shield = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.55, 0.44), new THREE.MeshStandardMaterial({ color: 0xc49b47, metalness: 0.58, roughness: 0.32 }))
  ...
  if (actor.actorType === 'npc') {
    const frost = new THREE.Mesh(new THREE.TorusGeometry(0.27, 0.03, 8, 24), new THREE.MeshBasicMaterial({ color: 0xa9e7ff, transparent: true, opacity: 0.75 }))
  ...
  const barBack = new THREE.Mesh(new THREE.PlaneGeometry(0.64, 0.075), new THREE.MeshBasicMaterial({ color: 0x261a1d, depthTest: false }))
  const ratio = Math.max(0.01, actor.hp / actor.maxHp)
  const barFill = new THREE.Mesh(new THREE.PlaneGeometry(0.6 * ratio, 0.045), new THREE.MeshBasicMaterial({ color: actor.faction === 'enemy' ? 0xf0626e : 0x70d58d, depthTest: false }))
  ...
  if (actor.shield > 0) {
    const shieldRing = new THREE.Mesh(
      new THREE.TorusGeometry(0.4, 0.035, 8, 32),
...
export function ThreeBoard({
  ...
  useEffect(() => {
    ...
    const hoverRing = new THREE.Mesh(hoverGeometry, new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.55, depthWrite: false }))
    ...
    const handlePointerMove = (eventValue: PointerEvent) => {
      ...
      renderer.domElement.style.cursor = coord ? 'pointer' : 'default'
      hoverRing.visible = Boolean(coord)
      if (coord) hoverRing.position.copy(worldPosition(coord, state, 0.18))
    ...
    const handleClick = (eventValue: PointerEvent) => {
      const coord = findCoord(eventValue)
  ...
  useEffect(() => {
    ...
    if (!camera) return
    const angle = Math.PI / 4 + cameraQuarter * Math.PI / 2
    camera.position.set(Math.cos(angle) * 13.5, 13, Math.sin(angle) * 13.5)
  ...
  useEffect(() => {
    ...
    for (const cell of state.cells) {
      ...
      const edges = new THREE.LineSegments(
        ...
        new THREE.LineBasicMaterial({ color: temperatureColors[Math.max(0, Math.min(6, cell.groundTemp + 3))], transparent: true, opacity: 0.55 }),
      )
      tile.add(edges)
      ...
      if (cell.moisture === 2 && cell.groundFill !== 'water') {
        const puddle = createOverlay(0x6db8d2, 0.24, 0.095)
        puddle.scale.set(0.55, 0.55, 0.55)
      ...
      if (cell.tags.includes('Shelter')) {
        ...
        const pillar = new THREE.Mesh(new THREE.CylinderGeometry(0.13, 0.18, 0.48, 12), new THREE.MeshStandardMaterial({ color: 0xd7c79b, roughness: 0.75 }))
      ...
      if (showSky && cell.skyFill === 'cloud') {
        ...
        content.add(cloud)
        const cloudShadow = createOverlay(0x24354d, 0.28, 0.12)
        cloudShadow.scale.set(0.75, 0.75, 0.75)
    ...
    if (event?.target) {
      const color = event.kind === 'cool' ? 0x5cc7ff : event.kind === 'heat' ? 0xff814b : event.kind === 'attack' ? 0xff4e5d : 0x73e6ac
  ...
  return <div className="three-board-host" ref={hostRef} aria-label="Three.js 2.5D game board" />
}
