...
export const REFLECTED_ACTOR_CONFLICT_RULE = 'reflected-actor-current-m-exchange-v1'
export const WALL_TRAVEL_BUDGET_RULE = 'wall-cell-roundtrip-costs-one-v2'
...
function currentPlayerImpactM(plan, routeIndex, routeLength) {
  const baseM = plan.actionKind === 'basic'
    ? clampM(plan.beforeM ?? 0)
    : clampM(momentumLevel(plan.afterImpulseSpeed ?? plan.finalSpeed ?? 0))
  if (!(plan.collisions?.length > 0)) return baseM
  const progress = routeIndex / Math.max(1, routeLength - 1)
  const prior = plan.collisions
    .filter((entry) => entry.reflection && Number(entry.t ?? 0) <= progress + 1e-6)
    .sort((a, b) => Number(a.t ?? 0) - Number(b.t ?? 0))
  const latest = prior.at(-1)
  return latest && Number.isFinite(latest.afterM) ? clampM(latest.afterM) : baseM
}
...
function resolveStepwiseKnockback({
  ...
  const moveActor = (currentActorId, initialDirection, initialPower, depth = 0) => {
    ...
    const startHex = actor ? cloneHex(actor.hex) : null
    ...
    while (movedSteps < movementBudget && activeM > 0 && guard < movementBudget * 5 + 12) {
      ...
      if (impact) {
        ...
        events.push({
          kind: 'wall-crash', actorId: currentActorId, power: activeM,
          obstacleId: obstacle?.id ?? null,
          obstacleKind: boundary ? 'boundary' : obstacle?.kind ?? 'hard',
          geometryKind: impact.kind,
          from: cloneHex(actor.hex), cell: cloneHex(next), contactPoint: { ...impact.point },
          faceIds: [...(impact.faceIds ?? [])], surfaceGeometry: SURFACE_GEOMETRY_RULE,
          reflectionContinuation: continuationRule,
          wallCellPivot: Boolean(impact.wallCellPivot), wallCellTravelCost: impact.wallCellTravelCost ?? 0,
          wallAxis: impact.wallAxis ?? null, partial: movedSteps > 0,
        ...
        if (!mirror) {
          ...
          events.push({
            ...
            cell: cloneHex(actor.hex), attemptedCell: cloneHex(next), reflectedAxis: null, reflectedCell: null,
            beforeM, afterM: 0, surfaceGeometry: SURFACE_GEOMETRY_RULE,
            reflectionContinuation: continuationRule,
            wallCellPivot: Boolean(impact.wallCellPivot), wallCellTravelCost: impact.wallCellTravelCost ?? 0,
            travelBudgetRule: WALL_TRAVEL_BUDGET_RULE,
          })
        ...
        events.push({
          ...
          ambiguousVertexBranch: Boolean(mirror.ambiguousVertex), wallCellPivot: Boolean(impact.wallCellPivot),
    ...
    actor.axisId = activeDirection.id
    return { vacated: !sameHex(actor.hex, startHex), activeM, direction: activeDirection }
  }
...
export function resolveCellConflicts({
  ...
  if (plan.spatialMode !== 'discrete') {
    return {
      ...plan, actorStates, conflictEvents: [], pushAtomic: false,
  ...
  for (let index = 1; index < route.length; index += 1) {
    ...
    const impactM = currentPlayerImpactM(plan, index, route.length)
  ...
  if (!playerConflict) {
    return {
      ...plan,
      actorStates: actorStates.map(cloneActor), actorTrajectories, actorPlaybackWindows,
      conflictEvents, pushAtomic: false,
      finalState: { ...plan.finalState, actors: actorStates.map(cloneActor) },
    }
  }
  ...
  if (playerRoute.length <= 1) {
    ...
    samples = playerRoute.map((cell, index) => ({
      t: index / (playerRoute.length - 1), position: axialToWorld(cell),
  ...
  return {
    ...
    cellConflict: {
      ...
      atomic: false,
      resolution: 'stepwise-clipped-mirror-v3',
