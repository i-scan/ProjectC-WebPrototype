...
export function ThermalClockLab() {
  ...
  return (
    <main className="thermal-clock-lab" data-implementation={THERMAL_CLOCK_RULE} data-thermal-solver={inertialMode ? THERMAL_INERTIAL_SOLVER : THERMAL_CLOCK_SOLVER} data-thermal-dynamics-mode={dynamicsMode} data-thermal-inertial-rule={THERMAL_INERTIAL_RULE} data-thermal-at={THERMAL_CLOCK_AT_RULE} data-thermal-preview={THERMAL_CLOCK_PREVIEW_RULE} data-thermal-ghost={THERMAL_CLOCK_GHOST_RULE} data-thermal-adiabatic={diagnostics.adiabatic ? 'true' : 'false'} data-thermal-regime={diagnostics.regime} data-thermal-action={selectedAction} data-world-at={state.worldAt.toFixed(3)}>
      <header className="thermal-clock-header">
        <div><p>ProjectC · VAL-012 Thermal Clock Dual-Mode v1</p><h1>Thermal Clock Lab</h1></div>
        <div className="thermal-clock-headline">
          ...
          <div><span>Drift {inertialMode ? 'D' : 'V'}</span><strong>{formatThermal(visualState.drift, 2)}</strong></div>
          <div><span>{inertialMode ? 'Net Rate' : 'Regime'}</span><strong>{inertialMode ? formatThermal(diagramDiagnostics.netRate, 2) : diagnostics.regime}</strong></div>
          <div><span>{inertialMode ? 'Next Apex' : 'Period'}</span><strong>{inertialMode ? formatAt(nextInertialApexAt({ state: visualState, thermalConfig: config, inertialConfig, driveRate: 0, horizonAt: previewHorizon })) : (diagnostics.dampedPeriodAt ? formatAt(diagnostics.dampedPeriodAt) : '—')}</strong></div>
        </div>
      </header>
      ...
      <section className="thermal-clock-grid">
        <aside className="thermal-panel thermal-left">
          <section className="thermal-card thermal-pendulum-card"><div className="thermal-section-heading"><h2>Thermal Pendulum</h2><span>bob/history inside · Skip outside</span></div><ThermalPendulum state={visualState} config={diagramConfig} previousState={previousState} diagnostics={diagramDiagnostics} dynamicsMode={diagramMode} inertialConfig={diagramInertialConfig} /></section>
          <section className="thermal-card" data-thermal-state-debug><div className="thermal-section-heading"><h3>Actor Thermal State</h3><span>LAB DEBUG</span></div><RangeField label="Current T" value={state.temperature} min={-6} max={6} step={0.05} disabled={Boolean(playback)} onChange={(value) => updateStateField('temperature', value)} /><RangeField label={inertialMode ? 'Current Drift D' : 'Current Drift V'} value={state.drift} min={-3} max={3} step={0.05} disabled={Boolean(playback)} onChange={(value) => updateStateField('drift', value)} suffix="/AT" /><RangeField label="Set Point S" value={state.setPoint} min={-4} max={4} step={0.05} disabled={Boolean(playback)} onChange={(value) => updateStateField('setPoint', value)} /><p className="thermal-proxy-note">Set Point slider = <b>DEBUG / BUILD PROXY</b>. Environment does not rewrite S.</p></section>
        </aside>
...
{inertialMode && <div className="thermal-impact-events" data-thermal-impact-events="instant-drift-impulse-slots-v1">
  ...
  {inertialMode ? <>
    ...
    <RangeField label="Drift Half-Life HD" value={inertialConfig.driftHalfLifeAt} min={0.25} max={3} step={0.05} disabled={Boolean(playback)} onChange={(value) => setInertialConfig((current) => ({ ...current, driftHalfLifeAt: value }))} suffix="AT" />
    <label className="thermal-choice-label">Recovery Half-Life preset</label>
    <div className="thermal-preset-row" data-thermal-recovery-half-life-presets>{RECOVERY_HALF_LIFE_PRESETS.map((value) => <button type="button" key={value} className={Math.abs(inertialConfig.recoveryHalfLifeAt - value) < 1e-6 ? 'selected' : ''} disabled={Boolean(playback)} onClick={() => setInertialConfig((current) => ({ ...current, recoveryHalfLifeAt: value }))}>{value} AT</button>)}</div>
    <RangeField label="Recovery Half-Life HR" value={inertialConfig.recoveryHalfLifeAt} min={0.5} max={8} step={0.1} disabled={Boolean(playback)} onChange={(value) => setInertialConfig((current) => ({ ...current, recoveryHalfLifeAt: value }))} suffix="AT" />
  </> : <>
...
<section className="thermal-card thermal-diagnostics" data-thermal-diagnostics>
  <div className="thermal-section-heading"><h3>Derived Diagnostics</h3><span>{inertialMode ? THERMAL_INERTIAL_SOLVER : THERMAL_CLOCK_SOLVER}</span></div>
  <dl>{inertialMode ? <>
    <div><dt>λD</dt><dd>{formatNumber(diagnostics.lambdaD)}</dd></div>
    <div><dt>kR</dt><dd>{formatNumber(diagnostics.kR)}</dd></div>
    <div><dt>kE</dt><dd>{formatNumber(diagnostics.kE)}</dd></div>
    <div><dt>Drift D</dt><dd>{formatThermal(diagnostics.drift, 3)}</dd></div>
    <div><dt>Recovery</dt><dd>{formatThermal(diagnostics.recovery, 3)}</dd></div>
    <div><dt>Env Exchange</dt><dd>{formatThermal(diagnostics.environmentExchange, 3)}</dd></div>
    <div><dt>Net Rate</dt><dd>{formatThermal(diagnostics.netRate, 3)}</dd></div>
    <div><dt>Next Apex</dt><dd>{formatAt(nextInertialApexAt({ state, thermalConfig: config, inertialConfig, driveRate: 0, horizonAt: previewHorizon }))}</dd></div>
  </> : <>
    <div><dt>K</dt><dd>{formatNumber(diagnostics.K)}</dd></div><div><dt>Teq</dt><dd>{formatThermal(diagnostics.equilibriumTemperature, 3)}</dd></div><div><dt>cEff</dt><dd>{formatNumber(diagnostics.cEff)}</dd></div><div><dt>D</dt><dd>{formatNumber(diagnostics.discriminant)}</dd></div><div><dt>Regime</dt><dd>{diagnostics.regime}</dd></div><div><dt>Adiabatic</dt><dd>{diagnostics.adiabatic ? 'YES' : 'NO'}</dd></div><div><dt>Damped Period</dt><dd>{diagnostics.dampedPeriodAt ? formatAt(diagnostics.dampedPeriodAt) : '—'}</dd></div><div><dt>Decay / cycle</dt><dd>{Number.isFinite(diagnostics.amplitudeDecayPerCycle) ? `${(diagnostics.amplitudeDecayPerCycle * 100).toFixed(1)}%` : '—'}</dd></div>
  </>}</dl>
</section>
