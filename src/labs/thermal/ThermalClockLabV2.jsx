...
function pointOnTrack(angleDeg, radius = TRACK_RADIUS) {
...
function arcPath(startAngle, endAngle, radius = TRACK_RADIUS) {
  ...
  return Array.from({ length: steps + 1 }, (_, index) => {
    const t = index / steps
    const point = pointOnTrack(startAngle + (endAngle - startAngle) * t, radius)
...
function ThermalPendulum({ state, config, previousState, diagnostics }) {
  ...
  const current = pointOnTrack(currentAngle)
  ...
  const nextPoint = pointOnTrack(nextAngle)
  const arrow = Math.abs(nextAngle - currentAngle) > 0.15 ? arcPath(currentAngle, nextAngle, TRACK_RADIUS + 1) : ''
  const previousPoint = previousState
    ? pointOnTrack(angleForTemperature(previousState.temperature, state.setPoint, true))
    : null
  ...
  return (
    <div className="thermal-pendulum thermal-pendulum--compact" data-thermal-pendulum="skip-next-history-v3">
      ...
      <svg viewBox="0 0 300 172" role="img" aria-label="Thermal Pendulum with Skip next-AT arrow and previous-AT history ring">
        <defs>
          <marker id="thermal-skip-arrow-head" markerWidth="7" markerHeight="7" refX="5.7" refY="3.5" orient="auto" markerUnits="strokeWidth">
        ...
        {previousPoint && (
          <circle
            className="thermal-pendulum__history-ring"
            data-thermal-pendulum-history-ring="previous-at-v1"
            cx={previousPoint.x}
            cy={previousPoint.y}
            r="13.5"
          />
        ...
        {arrow ? (
          <path className="thermal-pendulum__skip-arrow" data-thermal-pendulum-skip-arrow="next-at-v1" d={arrow} markerEnd="url(#thermal-skip-arrow-head)" />
        ) : (
          <circle className="thermal-pendulum__skip-idle" data-thermal-pendulum-skip-arrow="next-at-v1" cx={nextPoint.x} cy={nextPoint.y} r="3.5" />
        )}
        <circle className="thermal-pendulum__skip-next" cx={nextPoint.x} cy={nextPoint.y} r="3" />
        ...
        <circle className={`thermal-pendulum__bob ${zoneClass(Math.round(clamp(state.temperature, -4, 4)))}`} cx={current.x} cy={current.y} r="11" />
        <circle className="thermal-pendulum__bob-core" cx={current.x} cy={current.y} r="3" />
...
function ThermalDiagram({ state, history, selectedFuture, skipFuture, futureGhosts, config, diagnostics, pastWindow, futureWindow, actionLabel, resolving }) {
  ...
  const visibleValues = [state.temperature, ...historySamples.map((sample) => sample.temperature), ...selectedFuture.map((sample) => sample.temperature), ...skipFuture.map((sample) => sample.temperature)].filter(Number.isFinite)
  ...
  let minT = Math.max(config.clampMin, Math.min(state.setPoint - 2.5, valueMin - 0.35))
  let maxT = Math.min(config.clampMax, Math.max(state.setPoint + 2.5, valueMax + 0.35))
  if (maxT - minT < 1) {
  ...
  const pointsFor = (samples) => samples
    .filter((sample) => Number.isFinite(sample.temperature) && Number.isFinite(sample.at))
    .map((sample) => `${xFor(sample.at).toFixed(2)},${yFor(sample.temperature).toFixed(2)}`)
    .join(' ')
  const xTicks = []
  ...
  return (
    <section className="thermal-card thermal-timeline thermal-diagram" data-thermal-diagram="history-live-forecast-v2">
      <div className="thermal-section-heading">
        <div><h3>Thermal Diagram</h3><p>Solid = actual history · dashed = live future from the current NOW.</p></div>
        <span>−{pastWindow}AT / +{futureWindow}AT</span>
      </div>
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Thermal Diagram: continuous history, current state and live future forecast">
        ...
        {xTicks.map((tick) => (
          <g key={`x-${tick}`}>
            <line className="thermal-timeline__grid" x1={xFor(tick)} x2={xFor(tick)} y1={pad.top} y2={height - pad.bottom} />
        ...
        <g data-thermal-integer-ghosts="diagram-live-v2">
          {futureGhosts.map((ghost) => (
            <g key={`future-${ghost.offset}`}>
              <circle className="thermal-timeline__future-marker" cx={xFor(ghost.at)} cy={yFor(ghost.temperature)} r="4" />
              <text className="thermal-timeline__future-label" x={xFor(ghost.at)} y={yFor(ghost.temperature) - 8} textAnchor="middle">+{ghost.offset}</text>
            </g>
          ))}
        </g>
...
function ParameterGuide({ diagnostics, config }) {
  const currentPeriod = diagnostics.adiabatic && diagnostics.regime === 'underdamped'
    ? `P ≈ ${formatAt(diagnostics.dampedPeriodAt)}`
    : 'Set kE=0 and remain underdamped to expose a clock period.'
  return (
    <section className="thermal-card thermal-parameter-guide" data-thermal-parameter-guide="v1">
      ...
      <dl className="thermal-parameter-list">
        ...
        <div><dt>Clamp / Playback</dt><dd>Debug display safety / visual speed only; neither is a core physical coefficient.</dd></div>
      </dl>
      <div className="thermal-period-help" data-thermal-period-guide="adiabatic-underdamped-v1">
        <b>Adiabatic period</b>
        <span><code>P = 2π / √(kS − cBase²/4)</code>. Raise kS → shorter period; keep cBase low for a near-simple harmonic clock.</span>
        <span>Current: kS {formatNumber(config.restoringK, 2)} · cBase {formatNumber(config.baseDamping, 2)} · {currentPeriod}</span>
      </div>
    </section>
  )
}
...
export function ThermalClockLab() {
  ...
  const [lastEvent, setLastEvent] = useState('Stage-1 baseline loaded. Heat II is selected; no world time has advanced.')
  ...
  const diagramSelectedStart = useMemo(
    () => playback ? copyState(visualState) : applyThermalImpulse(visualState, selectedImpulse),
    [playback, visualState, selectedImpulse],
  )
  const selectedFuture = useMemo(
    () => sampleFuture(diagramSelectedStart, diagramConfig, previewHorizon, visualState.worldAt),
    [diagramSelectedStart, diagramConfig, previewHorizon, visualState.worldAt],
  )
  const skipFuture = useMemo(
    () => playback ? [] : sampleFuture(visualState, config, previewHorizon, visualState.worldAt),
    [playback, visualState, config, previewHorizon],
  )
  const futureGhosts = useMemo(
    () => integerFuture(diagramSelectedStart, diagramConfig, previewHorizon, visualState.worldAt),
    [diagramSelectedStart, diagramConfig, previewHorizon, visualState.worldAt],
  )
  ...
  const reset = () => {
    ...
    setPlaybackSpeed(1)
    setHistory([])
    setLastEvent('Stage-1 baseline reset. Heat II selected; worldAt = 0.')
  }
  ...
  useEffect(() => {
    window.__PROJECTC_THERMAL_CLOCK__ = {
      snapshot: () => ({
        ...
        visualState: copyState(visualState),
      ...
      <section className="thermal-clock-grid">
        <aside className="thermal-panel thermal-left">
          <section className="thermal-card thermal-pendulum-card">
            <div className="thermal-section-heading"><h2>Thermal Pendulum</h2><span>Driving-style compact readout</span></div>
            <ThermalPendulum state={visualState} config={diagramConfig} previousState={previousState} diagnostics={diagramDiagnostics} />
          </section>
        ...
        <section className="thermal-center">
          <div className={`thermal-status ${playback ? 'is-resolving' : 'is-ready'}`}>
            <strong>{playback ? 'ACTION IN FLIGHT · DIAGRAM UPDATES FROM LIVE NOW' : 'READY · SELECT → FORECAST → COMMIT'}</strong>
            <span>{lastEvent}</span>
          </div>

          <ThermalDiagram state={visualState} history={history} selectedFuture={selectedFuture} skipFuture={skipFuture} futureGhosts={futureGhosts} config={diagramConfig} diagnostics={diagramDiagnostics} pastWindow={pastWindow} futureWindow={previewHorizon} actionLabel={action?.label ?? selectedAction} resolving={Boolean(playback)} />
          ...
          <section className="thermal-board-reserved thermal-board-reserved--compact" data-thermal-board-reserved="true">
            ...
            <div><p>RESERVED BOARD</p><h2>Future Trajectory Integration</h2><span>Thermal Dynamics remains isolated while the diagram and pendulum are evaluated.</span></div>
          </section>

          <section className="thermal-action-hand">
            <div className="thermal-hand-heading">
              <div><h2>Thermal Actions</h2><p>Selected card applies one Drift impulse now; the Thermal Diagram forecasts what follows under the current parameters.</p></div>
            ...
            <div className="thermal-action-row">
              {THERMAL_ACTIONS.map((entry) => (
                <button type="button" key={entry.id} data-thermal-card={entry.id} className={`thermal-action-card ${entry.id === selectedAction ? 'selected' : ''} ${entry.sign > 0 ? 'heat' : entry.sign < 0 ? 'cool' : 'skip'}`} disabled={Boolean(playback)} onClick={() => { setSelectedAction(entry.id); setLastEvent(`${entry.label} selected. Thermal Diagram updated; worldAt has not advanced.`) }}>
        ...
        <aside className="thermal-panel thermal-right">
          <section className="thermal-card" data-thermal-environment>
            ...
            <div className="thermal-preset-row">{ENVIRONMENT_PRESETS.map((preset) => <button type="button" key={preset.id} disabled={Boolean(playback)} onClick={() => applyEnvironmentPreset(preset)}>{preset.label}</button>)}</div>
          ...
          <section className="thermal-card" data-thermal-dynamics>
            <div className="thermal-section-heading"><h3>Dynamics</h3><span>period + damping</span></div>
            <div className="thermal-preset-row">{DYNAMICS_PRESETS.map((preset) => <button type="button" key={preset.id} disabled={Boolean(playback)} onClick={() => applyDynamicsPreset(preset)}>{preset.label}</button>)}</div>
          ...
          <section className="thermal-card thermal-diagnostics" data-thermal-diagnostics>
            ...
            <dl>
              <div><dt>K</dt><dd>{formatNumber(diagnostics.K)}</dd></div><div><dt>Teq</dt><dd>{formatThermal(diagnostics.equilibriumTemperature, 3)}</dd></div>
              <div><dt>cEff</dt><dd>{formatNumber(diagnostics.cEff)}</dd></div><div><dt>D</dt><dd>{formatNumber(diagnostics.discriminant)}</dd></div>
              <div><dt>Regime</dt><dd>{diagnostics.regime}</dd></div><div><dt>Adiabatic</dt><dd>{diagnostics.adiabatic ? 'YES' : 'NO'}</dd></div>
              <div><dt>Damped Period</dt><dd>{diagnostics.dampedPeriodAt ? formatAt(diagnostics.dampedPeriodAt) : '—'}</dd></div><div><dt>Decay / cycle</dt><dd>{Number.isFinite(diagnostics.amplitudeDecayPerCycle) ? `${(diagnostics.amplitudeDecayPerCycle * 100).toFixed(1)}%` : '—'}</dd></div>
            </dl>
          </section>

          <section className="thermal-card" data-thermal-preview-controls>
            <div className="thermal-section-heading"><h3>Diagram / Playback</h3><span>display window</span></div>
            <label className="thermal-choice-label">Past history shown</label>
            <div className="thermal-choice-row" role="group" aria-label="Past History Window" data-thermal-past-window>{PAST_WINDOWS.map((windowAt) => <button type="button" key={windowAt} className={pastWindow === windowAt ? 'selected' : ''} disabled={Boolean(playback)} onClick={() => setPastWindow(windowAt)}>{windowAt} AT</button>)}</div>
            <label className="thermal-choice-label">Future forecast shown</label>
            <div className="thermal-choice-row" role="group" aria-label="Preview Horizon" data-thermal-future-window>{THERMAL_PREVIEW_HORIZONS.map((horizon) => <button type="button" key={horizon} className={previewHorizon === horizon ? 'selected' : ''} disabled={Boolean(playback)} onClick={() => setPreviewHorizon(horizon)}>{horizon} AT</button>)}</div>
            ...
            <div className="thermal-session-buttons"><button type="button" disabled={Boolean(playback) || history.length === 0} onClick={undo}>Undo 1 step</button><button type="button" disabled={Boolean(playback)} onClick={reset}>Reset Lab</button></div>
          </section>
