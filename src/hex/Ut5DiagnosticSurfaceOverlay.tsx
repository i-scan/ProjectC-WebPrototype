...
import { labSurfaceLabel } from './coupledInertiaUt5'
import { hexWorldPosition } from './HexThreeBoard'

type RendererMode = '2d' | '3d'
type Props = { state: GameState; rendererMode: RendererMode; cameraResetToken: number }
...
function projectedPoint(point: THREE.Vector3, orbit: OrbitState, width: number, height: number) {
  ...
  return { x: (projected.x + 1) * 0.5 * width, y: (1 - projected.y) * 0.5 * height }
}
...
export function Ut5DiagnosticSurfaceOverlay({ state, rendererMode, cameraResetToken }: Props) {
  ...
  useEffect(() => {
    ...
    const attach = () => {
      ...
      if (!canvas) { retryFrame = requestAnimationFrame(attach); return }
      ...
      const handleUp = (event: PointerEvent) => { if (dragRef.current.pointerId === event.pointerId) dragRef.current.active = false }
    ...
    return () => { cancelAnimationFrame(retryFrame); cleanupCanvas?.(); observer.disconnect() }
  }, [rendererMode])
  ...
  const markers = useMemo(() => {
    ...
    return state.cells.flatMap((cell) => {
      ...
      return [{ key: `${cell.coord.x},${cell.coord.y}`, label, x: rect.left - frameRect.left + rect.width * 0.5, y: rect.top - frameRect.top + rect.height * 0.5 }]
    })
  }, [rendererMode, revision, state])

  return (
    <div ref={overlayRef} className="ut5-diagnostic-surface-overlay" aria-label="UT5 diagnostic surfaces" style={{ position: 'absolute', inset: 0, zIndex: 15, pointerEvents: 'none', overflow: 'hidden' }}>
      {markers.map((marker) => (
        <div key={marker.key} data-surface-label={marker.label} style={{ position: 'absolute', left: marker.x, top: marker.y, transform: 'translate(-50%, -50%)', display: 'flex', alignItems: 'center', gap: 4, padding: '3px 6px', border: '1px solid rgba(242, 200, 90, .9)', borderRadius: 6, background: 'rgba(14, 21, 32, .9)', color: '#fff0a8', fontSize: 9, fontWeight: 700, lineHeight: 1, boxShadow: '0 0 10px rgba(242, 200, 90, .28)', whiteSpace: 'nowrap' }}>
          <strong aria-hidden="true">{glyphFor(marker.label)}</strong><span>{marker.label}</span>
        </div>
      ))}
    </div>
  )
