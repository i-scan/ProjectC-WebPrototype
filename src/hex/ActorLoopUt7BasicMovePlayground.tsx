...
export function ActorLoopUt7BasicMovePlayground() {
  ...
  return (
    <>
      <main className="visual-prototype hex-prototype coupled-inertia-lab ut4-hex-layout ut6-actor-loop ut7-actor-loop" data-ruleset="VAL-012-UT7-candidate" data-implementation="inertia-driving-navigation-v4">
        <header className="visual-hud ut4-hud">
          <div className="visual-brand"><p className="eyebrow">ProjectC · VAL-012-UT7 · Inertia Driving</p><h1>Basic Move Navigation Playground</h1></div>
        ...
        <section className="visual-layout ut4-visual-layout">
          ...
          <section className="visual-board-column hex-board-column ut4-board-column">
            ...
            <details className="ut4-diagnostics"><summary>UT7 Event Log · {lab.logs.length} · Move / Thermal / Build</summary><div className="ut4-diagnostics-body"><div className="ut4-log-list">{lab.logs.length === 0 && <p className="ut4-empty">选择远处 Target，观察完整多 AT 路线；每一行仍是一个真实 1 AT 的 Cell-step、Redirect、M 与 Axis 结算。</p>}{lab.logs.map((entry) => <article key={entry.id}><header><strong>{entry.timeAt.toFixed(1)} AT · {entry.action}</strong><span>{entry.behavior} / {entry.thermalIntent}</span></header><p>{axisLabel(entry.beforeSpatial.axis)} M{entry.beforeSpatial.level} → {axisLabel(entry.afterSpatial.axis)} M{entry.afterSpatial.level} · T {entry.beforeThermal.temperature.toFixed(2)}→{entry.afterThermal.temperature.toFixed(2)}</p><small>{entry.detail}</small></article>)}</div><div className="ut4-test-strip"><strong>UT7 Basic Move</strong><span>1 command = route AT</span><span>Final Target navigation</span><span>1AT inertia edges</span><span>Breakaway</span><span>Side / Domain</span></div></div></details>
          </section>

          <aside className="visual-panel visual-right-panel ut4-debug-panel ut6-debug-panel ut7-debug-panel">
            <section><div className="visual-section-heading"><h3>Momentum Presets</h3><span>navigate to target</span></div><div className="ut6-preset-grid ut7-preset-grid">{(['neutral', 'm1-east', 'm2-east', 'm3-east', 'cold-down'] as Ut7Preset[]).map((preset) => <button key={preset} onClick={() => usePreset(preset)}>{preset}</button>)}</div><small className="ut6-note">M1/M2/M3 继续决定每个 1AT 边的真实运动；远程 Target 只把多个合法 1AT 边串成完整导航路线。</small></section>
            ...
            <section><div className="visual-section-heading"><h3>Experiment Settings</h3><span>candidate values</span></div><label className="ut4-select-row"><span>Natural Start</span><select value={settings.naturalBuildStartMode} onChange={(event) => setSettings((current) => ({ ...current, naturalBuildStartMode: event.target.value as NaturalBuildStartMode }))}><option value="axis-first">Axis First</option><option value="immediate-m1">Immediate M1</option></select></label><label className="ut4-select-row"><span>Launch/Brake Min</span><select value={settings.launchBrakeMinM} onChange={(event) => setSettings((current) => ({ ...current, launchBrakeMinM: Number(event.target.value) as 1 | 2 }))}><option value="1">M1</option><option value="2">M2</option></select></label><div className="ut6-toggle-list"><button className={settings.buildAfterConversionSameAt ? 'active' : ''} onClick={() => setSettings((current) => ({ ...current, buildAfterConversionSameAt: !current.buildAfterConversionSameAt }))}>Conversion same-AT Build · {settings.buildAfterConversionSameAt ? 'ON' : 'OFF'}</button><button className={settings.hotSideBreakawayAssistEnabled ? 'active' : ''} onClick={() => setSettings((current) => ({ ...current, hotSideBreakawayAssistEnabled: !current.hotSideBreakawayAssistEnabled }))}>Hot Side Breakaway Assist · {settings.hotSideBreakawayAssistEnabled ? 'ON' : 'OFF'}</button></div><small className="ut6-note">Side 决定 cap；Domain 决定 Build efficiency。Navigation 会连续结算所需 AT；每个 AT 仍使用同一套 Basic Move 规则。</small></section>
