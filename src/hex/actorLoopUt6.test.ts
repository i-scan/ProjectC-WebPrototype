...
describe('VAL-012-UT6 Actor Loop v0', () => {
  it('T1 compares Axis First and Immediate M1 using behavior continuity, not action IDs', () => {
    ...
    immediate = basicMovePlan(immediate, 'W', settings({ naturalBuildStartMode: 'immediate-m1' })).result
    expect(playerSpatial(immediate)).toEqual({ level: 1, axis: horizontalAxis('W') })

    let axisFirst = createActorLoopState()
    axisFirst = basicMovePlan(axisFirst, 'W', settings({ naturalBuildStartMode: 'axis-first' })).result
    expect(playerSpatial(axisFirst)).toEqual({ level: 0, axis: horizontalAxis('W') })
    axisFirst = basicMovePlan(axisFirst, 'W', settings({ naturalBuildStartMode: 'axis-first' })).result
    expect(playerSpatial(axisFirst)).toEqual({ level: 1, axis: horizontalAxis('W') })

    axisFirst = stepWorldPlan(axisFirst, settings({ naturalBuildStartMode: 'axis-first' })).result
    const afterWait = basicMovePlan(axisFirst, 'W', settings({ naturalBuildStartMode: 'axis-first' })).result
  ...
  it('T2 Basic Move spends M1 for Move2 and cannot refund it in the same AT', () => {
    ...
    state = setThermalDebug(state, { temperature: 4, setPoint: 2, drift: 0 })
    state = setSpatialDebug(state, 'player', createSpatialState(1, horizontalAxis('W')))
    const plan = basicMovePlan(state, 'W', settings())
    expect(plan.valid).toBe(true)
    expect(plan.path).toHaveLength(2)
    expect(playerSpatial(plan.result)).toEqual({ level: 0, axis: horizontalAxis('W') })
  ...
  it('T3 Basic Move spends one layer from M2 and leaves M1', () => {
    let state = createActorLoopState()
    state = setSpatialDebug(state, 'player', createSpatialState(2, horizontalAxis('W')))
    const plan = basicMovePlan(state, 'W', settings())
    expect(plan.path).toHaveLength(2)
    expect(playerSpatial(plan.result)).toEqual({ level: 1, axis: horizontalAxis('W') })
  })
  ...
  it('Natural Build caps at M1 in Neutral while matching Cold Domain can grow Down to M3', () => {
    ...
    cold = setThermalDebug(cold, { temperature: -4, setPoint: -2, drift: 0 })
  ...
  it('matching Hot continuous Drive can grow Horizontal M beyond Natural cap', () => {
    let state = createActorLoopState()
    state = setThermalDebug(state, { temperature: 4, setPoint: 2, drift: 0 })
    const plan = drivePlan(state, 'W', settings({ drivePreservesMomentum: true, driveContinuousTraversal: true }))
    expect(plan.valid).toBe(true)
    expect(plan.atCost).toBe(2)
    expect(playerSpatial(plan.result).level).toBeGreaterThanOrEqual(2)
    expect(axisLabel(playerSpatial(plan.result).axis)).toBe('Axis W')
  })
  ...
  it('T7 Launch / Brake MinM can compare 1 vs 2 without changing the core conversion rule', () => {
    ...
    expect(launchPlan(launchState, 'W', settings({ launchBrakeMinM: 2 })).valid).toBe(false)
    const launch = launchPlan(launchState, 'W', settings({ launchBrakeMinM: 1 }))
    expect(launch.valid).toBe(true)
    expect(launch.path).toHaveLength(1)
    expect(playerSpatial(launch.result)).toEqual({ level: 0, axis: horizontalAxis('W') })

    let brakeState = createActorLoopState()
    brakeState = setSpatialDebug(brakeState, 'player', createSpatialState(2, horizontalAxis('W')))
  ...
  it('T8 Conversion same-AT Build remains an explicit A/B independent of Basic Spend Lock', () => {
    ...
    const off = launchPlan(base, 'W', settings({ buildAfterConversionSameAt: false })).result
    const on = launchPlan(base, 'W', settings({ buildAfterConversionSameAt: true })).result
  ...
  it('T10 Incoming Momentum follows cancel -> remaining axis -> movement exactly', () => {
    ...
    const r0 = applyIncomingMomentum(m0, 'player', 'W', 1)
    expect(r0.moved).toBe(1)
    expect(playerSpatial(r0.state)).toEqual({ level: 1, axis: horizontalAxis('W') })
    ...
    const r1 = applyIncomingMomentum(down1, 'player', 'W', 1)
    expect(r1.moved).toBe(0)
    expect(playerSpatial(r1.state)).toEqual({ level: 0, axis: null })

    const r3 = applyIncomingMomentum(down1, 'player', 'W', 3)
    expect(r3.moved).toBe(2)
    expect(playerSpatial(r3.state)).toEqual({ level: 2, axis: horizontalAxis('W') })
  })
  ...
  it('T11 AT0 changes immediate state but freezes world time, Thermal and Build', () => {
    ...
    const target = state.game.actors.find((actor) => actor.alive && hexDistance(getPlayer(state.game).position, actor.position) === 1)!
  ...
  it('T13 Drive Preserve / Continuous Traversal are experiment settings, not hidden rules', () => {
    ...
    state = setThermalDebug(state, { temperature: 4, setPoint: 2, drift: 0 })
    state = setSpatialDebug(state, 'player', createSpatialState(2, horizontalAxis('W')))
    const preserve = drivePlan(state, 'W', settings({ drivePreservesMomentum: true, driveContinuousTraversal: true }))
    const consume = drivePlan(state, 'W', settings({ drivePreservesMomentum: false, driveContinuousTraversal: false }))
