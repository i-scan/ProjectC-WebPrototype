...
function createActor() {
  ...
  const base = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xe9d6a7, roughness: 0.55 }),
  )
  base.position.y = 0.08
  const body = new THREE.Mesh(
    new THREE.CylinderGeometry(0.21, 0.28, 0.58, 14),
    new THREE.MeshStandardMaterial({ color: 0x4ba7df, roughness: 0.48, metalness: 0.04 }),
  )
  body.position.y = 0.42
  const head = new THREE.Mesh(
    new THREE.SphereGeometry(0.19, 14, 10),
    new THREE.MeshStandardMaterial({ color: 0x4ba7df, roughness: 0.48 }),
  )
  ...
  for (let index = 0; index < 3; index += 1) {
    const dot = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color: 0x263342, transparent: true, opacity: 0.55 }),
    )
...
function updateMomentumDots(actor, level) {
  ...
  dots.forEach((dot, index) => {
    ...
    dot.material.color.setHex(active ? 0xc978ff : 0x263342)
...
function createMountain(cell) {
  ...
  const rock = new THREE.MeshStandardMaterial({ color: ridge ? 0x555c64 : 0x626a73, roughness: 0.94, metalness: 0.02, flatShading: true })
...
export function Board3D({
  ...
  const momentumArrowRef = useRef(null)
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(0x111a25, 12, 27)
    ...
    const sun = new THREE.DirectionalLight(0xffe7c0, 2.0)
    ...
    scene.add(boardGroup, interaction, actor, momentumArrow)
    sceneRef.current = scene
    cameraRef.current = camera
    boardGroupRef.current = boardGroup
    interactionRef.current = interaction
    actorRef.current = actor
    momentumArrowRef.current = momentumArrow
    ...
    const render = () => {
      ...
      const arrow = momentumArrowRef.current
      if (arrow) {
  ...
  useEffect(() => {
    ...
    for (const cell of cells) {
      ...
      if (cell.groundFill === 'water') {
        const water = createHexOverlay(0x5ec7df, 0.4, 0.058, HEX_RADIUS * 0.86)
        water.position.x = center.x
        water.position.z = center.z
        boardGroup.add(water)
      }
      if (cell.groundFill === 'ice') {
        const ice = createHexOverlay(0xc9f4ff, 0.52, 0.105, HEX_RADIUS * 0.87)
      ...
      if (cell.groundFill === 'fire') {
        for (let index = 0; index < 3; index += 1) {
          const flame = new THREE.Mesh(
            ...
            new THREE.MeshBasicMaterial({ color: index === 1 ? 0xffdf70 : 0xff7040, transparent: true, opacity: 0.88 }),
          )
      ...
      if (showWeather && cell.rain) {
        for (let index = 0; index < 7; index += 1) {
          const material = new THREE.MeshBasicMaterial({ color: 0x7fdcff, transparent: true, opacity: 0.67, depthWrite: false })
  ...
  useEffect(() => {
    ...
    const line = new THREE.Line(
      ...
      new THREE.LineBasicMaterial({
        color: previewPlan.collisions.length ? 0xffa45b : previewPlan.spatialMode === 'discrete' ? 0xf2c85a : 0x65d8ff,
        transparent: true,
        opacity: 0.94,
      }),
    )
  ...
  useEffect(() => {
    ...
    if (selectedAimHex) {
      ...
      const selected = createHexOverlay(0xf7d06e, 0.28, 0.14)
    ...
    if (hoverHex) {
      ...
      const reticle = createTargetReticle(previewPlan?.valid ? 0xf2c85a : 0xff6f6f, 0.18)
